...
STATE_CHOICES = [(s, s) for s in ['Init', 'GenerateRequest', 'RequestSent', 'ProcessResponse', 'Complete']]
...
class Transaction(models.Model):
	...
	AmountSettlement = models.DecimalField(decimal_places=2, max_digits=12, null=True, blank=True)
	CurrencyInput = models.CharField(max_length=4, choices=CURRENCY_CHOICES)
	CurrencySettlement = models.CharField(max_length=4, choices=CURRENCY_CHOICES, null=True, blank=True)
	...
	TxnMac = models.TextField(null=True, blank=True) # No info on max_length given in the documentation
	created = models.DateTimeField(auto_now_add=True)
	state = models.CharField(max_length=15, choices=STATE_CHOICES, default='Init')
	...
	def __init__(self, *args, **kwargs):
		...
		# Currency code must be provided either within transaction or in settings.
		if not kwargs.get('CurrencyInput'):
			try:
				kwargs['CurrencyInput'] = getattr(settings, 'PXPAY_CURRENCY')
			except AttributeError:
				raise KeyError("No currency set. Please provide CurrencyInput or specify PXPAY_CURRENCY in settings")
		return super(Transaction, self).__init__(*args, **kwargs)

	def __unicode__(self):
		return '%s' % self.TxnId

	# Generating unique txn_id. Making sure it is actually unique.
